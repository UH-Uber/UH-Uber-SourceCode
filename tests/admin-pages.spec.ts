...
test('Admin Pages', async ({ page }) => {
  ...
  await expect(
    page.getByRole('link', { name: 'Next.js Application Template' }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('button', { name: 'admin@foo.com' }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('heading', { name: 'List Stuff Admin' }),
  ).toBeVisible();
  await expect(
    page.getByRole('heading', { name: 'List Users Admin' }),
  ).toBeVisible();
});
